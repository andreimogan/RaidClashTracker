...
Web dashboard for a RAID: Shadow Legends clan's **Hydra Clash** and **Chimera Clash** performance — per-member keys used, damage, participation, and week-over-week trends. The app runs on the owner's machine (`npm run dev`); the database is hosted. **Reads are public; every write is admin-only** — one account, signed in at `/login`, authorized by an `ADMIN_EMAIL` match (see `docs/reference/auth.md`).

Stack: **Next.js 16 (App Router, TypeScript) · Tailwind CSS v4 · Recharts · lucide-react · Supabase Postgres 17 via `postgres` (postgres.js)** reached over the pooled (`:6543`) `DATABASE_URL` with `prepare: false`, plus **Supabase Auth via `@supabase/ssr`** (`2.112.3` / `0.12.4`) for sign-in only — never for data. Deployable to Vercel with the four env vars; nothing is deployed yet (Phase 4).
...
- `app/` — routes: `/` (overview), `/hydra`, `/chimera` (per-clash detail), `/timeline` (weekly totals), `/members` + `/members/[memberId]` (roster/detail), `/settings` (clan info, data management, import tab), `/import`, `/login` (sign-in; **deliberately absent from the sidebar** — see `docs/reference/auth.md`)
