...
- Never break the demo fallback: it fires only when **no connection string resolves** — neither `DATABASE_URL` nor `POSTGRES_URL` holds a non-blank value (`databaseUrl()` is `null`) — or a query hits `42P01`. So after your change a freshly-migrated-but-empty database must still render **genuinely empty** rather than demo data.
