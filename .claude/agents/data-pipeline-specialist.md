...
description: Use to implement tasks that touch the data-ingest pipeline — lib/parse.ts, lib/import.ts, lib/persist.ts, the import/backup/restore API routes, or the CLI import scripts. Dispatched instead of a generic flow-implementer for those slices, one per ownership group, after plan approval.
...
## Purpose
Build ingest-pipeline slices correctly: every data path (in-app JSON import, CSV file, member week edit, backup/restore) funnels through parse → normalize → persist, and the replace/upsert semantics are preserved exactly.
...
- Invariants you must not break (details in `.claude/rules/data-pipeline.md`):
  ...
  - The CSV file import and the member week edit are both **upsert**; the member week edit additionally requires the member and week to already exist.
...
- Run `npm run build` (and exercise the relevant import path against a throwaway Postgres schema — never the live clan tables — when feasible) before finishing; fix what you break.
